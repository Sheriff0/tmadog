	...
	def __getitem__ (self, sla):
		...
		if s in self and (not v or v not in self):
			...
			if self.traverse_deps:
                                #NOTE process dependencies (including the first below) and store the index of volatilities availabe in the cache for them to be cleaned-up
				deps = [s]
                                s1 = None;
				if not self.webmap [s]['path'].startswith (('/', 'http')):
					while True:
						dep = self.webmap [deps [-1]]['requires'].split(",")
						...
						if not dep:
							break
                                                    ...
                                                    brk = False;
                                                    for s1 in dep:
                                                        if self.webmap [s1]['path'].startswith (('/', 'http')):
                                                            deps.append (s1);
                                                            brk = True;
                                                            break

                                                        vols = self.webmap [s1]['volatile'].split(",");
                                                        brk1 = False;
                                                        for v in vols:
                                                            if v not in self and s1 in self:
